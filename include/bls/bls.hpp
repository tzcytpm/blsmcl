...
public:
	...
	/* WL get hex format */
	void getHexStr(std::string& str, int ioMode = 0) const
	{
		str.resize(1024);
		size_t n = blsSecretKeyGetHexStr(&str[0], str.size(), &self_);
		if (n == 0) throw std::runtime_error("blsSecretKeyGetHexStr");
		str.resize(n);
	}

	void SetHexStr(std::string& str, int ioMode = 0) const
	{
		str.resize(1024);
		//int ret = blsSecretKeySetHexStr(&self_, str.c_str(), str.size());
		//if (ret != 0) throw std::runtime_error("blsSecretKeySetHexStr");
	}
	void setStr(const std::string& str, int ioMode = 0)
	{
		int ret = mclBnFr_setStr(&self_.v, str.c_str(), str.size(), ioMode);
...
public:
	...
	/* WL get signature hex format */
	void getHexStr(std::string& str, int ioMode = 0) const
	{
		str.resize(1024);
		size_t n = blsSignatureGetHexStr(&str[0], str.size(), &self_);
		if (n == 0) throw std::runtime_error("blsSignatureGetHexStr");
		str.resize(n);
	}
	void SetHexStr(std::string& str, int ioMode = 0) const
	{
		str.resize(1024);
		//int ret = blsSignatureSetHexStr(&self_, str.c_str(), str.size());
		//if (ret != 0) throw std::runtime_error("blsSignatureSetHexStr");
	}
